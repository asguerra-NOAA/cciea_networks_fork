...
if(!require("PNWColors")) {devtools::install_github("jakelawlor/PNWColors"); library(PNWColors)}
```
<br>
...
png(
  here::here(
    pngdir,
    "Node Strength of Non-DTS Groundfish in Fisheries Participation Networks 2004-2025.png"
  ),
  res = 200,
  height = 1500,
  width = 1700
...
png(
  here::here(
    pngdir,
    "Importance of Non-DTS Groundfish in Fisheries Participation Networks 2004-2025.png"
  ),
  res = 200,
  height = 1500,
  width = 1700
...
# allports_sorted <- dat2 %>% left_join(iopac_mean_lat, by = c('Port Group' = 'IOPAC')) %>% dplyr::select(`
Port
Group
`, mean_lat) %>% arrange(-mean_lat) %>% select(`
Port
Group
...
Port
Group
`, levels=iopac_mean_lat)

# main metric plot
fig_ed <- ggplot(dat2 %>% filter( `
Port
Group
...
png(
  here::here(
    pngdir,
    paste0(
      "Edge Density in Fisheries Participation Networks 2005-2025",
      "_",
      Sys.Date(),
      ".png"
    )
...
fig_n <- ggplot(
  dat2 %>%
    filter(
      `Port Group` != "Other Coastal Washington" &
        `Port Group` != "Unknown Ports" &
        `Port Group` != "Columbia River"
    ),
  ...
  scale_colour_viridis(discrete = TRUE) +
  scale_x_continuous(
    breaks = seq(min(dat2$Year), max(dat2$Year), by = 2),
    labels = seq(min(dat2$Year), max(dat2$Year), by = 2)
  ) +
  #ylim(c(0,1)) +
  labs(
    title = 'Number of Fisheries in Fisheries Participation Networks',
    subtitle = '2004-2025',
    x = "Year",
    y = metric_label
  ) +
