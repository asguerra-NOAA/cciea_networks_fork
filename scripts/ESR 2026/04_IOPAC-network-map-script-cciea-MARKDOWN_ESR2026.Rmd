...
esr_dir = paste0(
  "/Users/ana.guerra/Documents/CONFIDENTIAL/Outputs/CCIEA Networks/Figures/"
) # jameal
...
#fig_path
thisyearsESRimages <- list.files(
  here::here(pngdir_ports),
  paste0("2025_circular_compare_", most_recent_igraph)
)

wa_fig_path <- here::here(
  'data/networks/participation_vessel_ports/plots/comparable/'
)
...
box_iopac <- function(basemap, box_filter, region) {
  iopac_rect <- basemap +
    geom_point(
      data = subset(box, map_version %in% region),
      aes(x = PRIMARY_LONGITUDE, y = PRIMARY_LATITUDE, fill = STATE),
      colour = "black",
      pch = 21,
      size = 2
    ) +
    geom_text_repel(
      aes(x = PRIMARY_LONGITUDE, y = PRIMARY_LATITUDE, label = IOPAC),
      data = subset(box, map_version %in% region),
      size = 2.15, # AG changed text size 010425
      box.padding = 0,
      point.padding = 0.25,
      hjust = -.25
    ) +
    ...
    geom_segment(
      data = subset(box_filter, segment_line == 'upper_right'),
      aes(
        x = PRIMARY_LONGITUDE,
        y = PRIMARY_LATITUDE,
        xend = xmax,
    ...
    geom_segment(
      data = subset(box_filter, segment_line == 'lower_right'),
      aes(
        x = PRIMARY_LONGITUDE,
        y = PRIMARY_LATITUDE,
        xend = xmax,
    ...
    geom_segment(
      data = subset(box_filter, segment_line == 'upper_left'),
      aes(
        x = PRIMARY_LONGITUDE,
        y = PRIMARY_LATITUDE,
        xend = xmin,
    ...
    geom_segment(
      data = subset(box_filter, segment_line == 'lower_left'),
      aes(
        x = PRIMARY_LONGITUDE,
        y = PRIMARY_LATITUDE,
        xend = xmin,
...
CW_map <- ggdraw(box_iopac(
  ...
  box_filter('CW'),
  c('WA', 'OR', 'NCA', 'SCA')
)) + # AG changed longitude limits 010425
  draw_image(
    img_ps,
    x = .06,
    y = .66,
...
ggsave(
  here::here(
    pngdir_ports,
    paste0('iopac-node_full-coast_rect_font_', thisyear, '.png')
  ),
  CW_map,
  width = 6,
  height = 10,
...
if (esr_output == TRUE) {
  ggsave(
    paste0(esr_dir, 'fpn-coast-', thisyear, '.png'),
    CW_map,
    width = 6,
    height = 10,
    dpi = 600
...
WA_map <- ggdraw(box_iopac(
  base_map_iopac("washington", c(-130, -117), c(NA, NA)),
  box_filter('WA'),
  'WA'
)) +
  draw_image(
...
ggsave(
  here::here(pngdir_ports, paste0('iopac-node_WA_rect_', thisyear, '.png')),
  WA_map,
  width = 6,
  height = 6,
  dpi = 600
...
if (esr_output == TRUE) {
  ggsave(
    paste0(esr_dir, 'fpn-wa-', thisyear, '.png'),
    WA_map,
    width = 6,
    height = 6,
    dpi = 600
...
OR_map <- ggdraw(box_iopac(
  base_map_iopac("oregon", c(-130, -117), c(NA, NA)),
  box_filter('OR'),
  'OR'
)) +
  draw_image(
...
ggsave(
  here::here(pngdir_ports, paste0('iopac-node_OR_rect_', thisyear, '.png')),
  OR_map,
  width = 6,
  height = 6,
  dpi = 600
...
if (esr_output == TRUE) {
  ggsave(
    paste0(esr_dir, 'fpn-or-', thisyear, '.png'),
    OR_map,
    width = 6,
    height = 6,
    dpi = 600
...
NCA_map <- ggdraw(box_iopac(
  base_map_iopac("california", c(-130, -117), c(NA, NA)),
  box_filter('NCA'),
  'NCA'
)) +
  draw_image(
...
ggsave(
  here::here(pngdir_ports, paste0('iopac-node_NCA_rect_', thisyear, '.png')),
  NCA_map,
  width = 6,
  height = 6,
  dpi = 600
...
if (esr_output == TRUE) {
  ggsave(
    paste0(esr_dir, 'fpn-nca-', thisyear, '.png'),
    NCA_map,
    width = 6,
    height = 6,
    dpi = 600
...
ggsave(
  here::here(pngdir_ports, paste0('iopac-node_SCA_rect_', thisyear, '.png')),
  SCA_map,
  width = 6,
  height = 6,
  dpi = 600
...
if (esr_output == TRUE) {
  ggsave(
    paste0(esr_dir, 'fpn-sca-', thisyear, '.png'),
    SCA_map,
    width = 6,
    height = 6,
    dpi = 600
...
ggsave(
  here::here(
    pngdir_ports,
    paste0(
      'All Fisheries Participation Networks ',
      thisyear,
      ' by IOPAC Port Group.png'
    )
  ),
  p_all,
  width = 6,
