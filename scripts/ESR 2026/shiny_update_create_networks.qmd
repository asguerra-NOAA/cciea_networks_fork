...
node_names <- read_csv(here::here(processdir,myfile5),
  ...
  rename(SPG_label = species_group_2_label2 )
```
...
```{r}
 ...
 dat <- fishtix %>%
      ...
      filter(crab_year == 2020 & IOPAC == "Monterey") 
...
```{r}
participation_network_tidy <- function(tickets, 
                                       min_vessels = 3, 
                                       min_contribution = 0.10, 
                                       min_rev_total = 5000, 
                                       min_rev_indiv = 500) {
  
  # 1. Clean and Filter Vessels
  vessel_summary <- tickets %>%
    filter(drvid != "NONE") %>%
    group_by(drvid, crab_year, SPGRPN2) %>%
    summarise(revenue = sum(adj_afi_revenue), .groups = "drop") %>%
    # Filter: Vessel must meet minimum individual fishery revenue
    filter(revenue >= min_rev_indiv) %>%
    group_by(drvid, crab_year) %>%
    # Filter: Vessel must meet total annual revenue
    mutate(total_annual_rev = sum(revenue)) %>%
    filter(total_annual_rev >= min_rev_total) %>%
    # Calculate proportions for weighting
    mutate(prop = revenue / total_annual_rev)
  
  # 2. Determine valid Fisheries (Nodes)
  valid_fisheries <- vessel_summary %>%
    group_by(SPGRPN2) %>%
    summarise(
      max_boats = n_distinct(drvid),
      med_prop = median(prop),
      .groups = "drop"
    ) %>%
    filter(max_boats >= min_vessels, med_prop >= min_contribution)
  
  # 3. Create Edges (Connectivity) using a Tidy Join
  # This replaces the nested for-loops with a self-join
  edges <- vessel_summary %>%
    filter(SPGRPN2 %in% valid_fisheries$SPGRPN2) %>%
    inner_join(vessel_summary, by = c("drvid", "crab_year"), suffix = c("_i", "_j")) %>%
    filter(SPGRPN2_i < SPGRPN2_j) %>% # Removes self-loops and duplicates
    mutate(weight = prop_i * prop_j * (revenue_i + revenue_j)) %>%
    group_by(from = SPGRPN2_i, to = SPGRPN2_j) %>%
    summarise(weight = sum(weight), .groups = "drop")
  
  # 4. Create Nodes with Attributes
  nodes <- valid_fisheries %>%
    rename(name = SPGRPN2) %>%
    mutate(importance = max_boats * med_prop)
...
```{r}
get_vertex_colors <- function(node_names) {
  case_when(
    node_names %in% c("1", "3", "19", "50", "55") ~ pal3[1],  # Groundfish
    node_names == "2"                             ~ pal3[2],  # Whiting
    node_names %in% c("20", "21")                 ~ pal3[3],  # Shrimp
    node_names == "30"                            ~ pal3[4],  # Salmon
    node_names == "35"                            ~ pal3[5],  # Tuna
    node_names %in% c("40", "45")                 ~ pal3[6],  # Pelagics
    node_names == "25"                            ~ pal3[7],  # Crab
    node_names == "70"                            ~ pal3[8],  # Squid
    node_names %in% c("60", "65")                 ~ pal3[10], # Shellfish
    TRUE                                          ~ pal3[11]  # Default
  )
}
```
...
net_stats_tidy <- function(g, y, stats = "both") {
  
  # ---  Pre-calculations ---
  # Basic properties used in multiple places
  v_names <- V(g)$name
  n_count <- vcount(g)
  e_count <- ecount(g)
  e_weights <- E(g)$weight
  
  # Normalized degree and relative strength
  deg_norm <- degree(g, normalized = TRUE)
  # Handle zero-edge case for strength sum to avoid NaN
  total_weight <- if(e_count > 0) sum(e_weights, na.rm = TRUE) else 1
  strength_rel <- strength(g) / total_weight

  # --- 2. Node-Level Stats ---
  if (stats %in% c("node", "both")) {
    
    # Handle betweenness weights (igraph treats weights as costs/distance)
    if (e_count < 1) {
      message("WARNING: No edges in this network.")
      btwn <- NA; btwn_inv <- NA; btwn_basic <- betweenness(g, normalized = TRUE)
    } else {
      # mean(weight)/weight or 1/weight to convert strength to cost
      btwn       <- betweenness(g, directed = FALSE, weights = mean(e_weights)/e_weights, normalized = TRUE)
      btwn_inv   <- betweenness(g, directed = FALSE, weights = 1/e_weights, normalized = TRUE)
      btwn_basic <- betweenness(g, directed = FALSE, weights = e_weights, normalized = TRUE)
    }

    node_stats <- tibble(
      year           = y,
      metier         = v_names,
      # Extract gear (string after underscore)
      gear           = str_split_fixed(v_names, "_", 2)[, 2],
      common_name    = V(g)$SPG_label,
      node_strength  = strength_rel,
      eigen          = eigen_centrality(g, weights = e_weights)$vector,
      btwn           = btwn,
      btwn_inverted  = btwn_inv,
      btwn_basic     = btwn_basic,
      deg_norm       = deg_norm
    )
  }

  # --- 3. Network-Level Stats ---
  if (stats %in% c("network", "both")) {
    
    # Cluster-based stats (Modularity)
    wtc        <- cluster_walktrap(g, weights = e_weights)
    m          <- modularity(g, membership(wtc))
    m_weighted <- modularity(g, membership(wtc), weights = e_weights)

    net_summary <- tibble(
      year           = y,
      N              = n_count,
      E              = e_count,
      # Edge weight stats (handling E=0/1 cases)
      avg_ew         = if(e_count > 0) mean(e_weights) else NA,
      sd_ew          = if(e_count > 1) sd(e_weights)   else NA,
      med_ew         = if(e_count > 0) median(e_weights) else NA,
      max_ew         = if(e_count > 0) max(e_weights) else 0,
      # Density and Centralization
      assort         = assortativity_degree(g, directed = FALSE),
      ed             = edge_density(g),
      ld             = e_count / n_count,
      mean_deg       = (2 * e_count) / n_count,
      nc             = net_centr(g),
      nc_weighted    = net_centr_weighted(g),
      # Modularity
      modularity     = m,
      mod_weighted   = m_weighted,
      beta_eff       = beta_eff(g)
    )
  }

  # --- 4. Return Logic ---
  if (stats == "node") return(node_stats)
  if (stats == "network") return(net_summary)
  
  return(list(node = node_stats, network = net_summary))
}
```
...
tickets <- fishtix %>%
      filter(crab_year == yr & IOPAC == port) # this line is just for running through for loop manually
...
min_rev_indiv <-  500
 ...
 # Clean and Filter Vessels
 vessel_rev <- tickets %>%
    filter(drvid != 'NONE') %>%
    group_by(drvid, crab_year, SPGRPN2) %>%
    summarise(revenue = sum(adj_afi_revenue, na.rm = TRUE), .groups = "drop") %>%
    filter(revenue >= min_rev_indiv)  %>% # Pre-filter individual revenue
  group_by(drvid, crab_year) %>%
  mutate(frac_rev = revenue / sum(revenue)) %>%
  ungroup()
   
# Valid Nodes (Fisheries)
nodes_df <- vessel_rev %>%
    group_by(name = SPGRPN2) %>%
    summarise(
      n_boats = n_distinct(drvid),
      total_rev = sum(revenue, na.rm = TRUE),
      percent_size = median(frac_rev, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    # Apply your threshold filters here
    filter(n_boats >= min_vessels) %>% 
    mutate(importance = total_rev * percent_size,
           colors = get_vertex_colors(name))
  
  # Create Edges (Connectivity)
edges_df <- vessel_rev %>%
    # CRITICAL: Only keep revenue for nodes that passed the filter
    filter(SPGRPN2 %in% nodes_df$name) %>%
    group_by(drvid) %>%
    mutate(frac_rev = revenue / sum(revenue)) %>%
    inner_join(vessel_rev, by = c("drvid", "crab_year"), suffix = c(".x", ".y")) %>%
    filter(SPGRPN2.x < SPGRPN2.y) %>% 
    mutate(weight = frac_rev.x * frac_rev.y * (revenue.x + revenue.y)) %>%
    group_by(from = SPGRPN2.x, to = SPGRPN2.y) %>%
    summarise(weight = sum(weight), .groups = "drop") %>%
    # Remove any NaN weights that might have slipped through
    filter(!is.na(weight))


  tg <- tbl_graph(nodes = nodes_df, edges = edges_df, directed = FALSE) 
...
tg_clean <- tg  %>%
    ...
    tickets %>% 
      group_by(SPGRPN2) %>% 
      summarise(vessels = n_distinct(drvid), .groups = "drop"),
    by = c("name" = "SPGRPN2")
  ) %>%
  left_join(node_names, by = c("name" = "SPGRPN2")) %>% 
    mutate(
      # Wrap long names for the circular layout
      display_name = str_wrap(SPG_label, width = 8),
      # Only show labels if there are vessels (prevents clutter)
      display_name = if_else(vessels == 0, "", display_name),
      # Create the (n) vessel count label
...
g_stats <- net_stats_tidy(tg_clean, y = y, stats = "both")

node_results <- g_stats$node %>%
        mutate(iopac = port)
      
network_results <- g_stats$network  %>%
        mutate(iopac = port)
...
```{r}
 ...
 np <-  ggraph(tg_clean, layout = 'linear', circular = TRUE) +
    # Draw edges: width represents 'connectivity' weight
    geom_edge_link(aes(width = weight), alpha = 0.2, color = "gray70", show.legend = FALSE) +
    # Draw nodes: size by importance, color by the 'colors' attribute you already set
    geom_node_point(aes(size = importance, color = colors), alpha = 0.9) +
    # Fishery labels: 'repel' automatically prevents overlapping
    geom_node_text(aes(label = display_name), repel = TRUE, size = 4, fontface = "bold") +
    # Vessel labels: smaller text near the nodes
    geom_node_text(aes(label = vessel_label), size = 3, color = "gray30", vjust = 2) +
    # Styling
    ...
    labs(
      title = paste(unique(V(tg_clean)$p), unique(V(tg_clean)$year))
      )+
   theme(
